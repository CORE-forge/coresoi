...
To compute indicator `ind_1`, we need to map each function argument to a column in the project partner's database.
...
To map these arguments to columns in the project partner's database, we need to ensure that the partner data has a column named `stat_unit`, which should only be the companies that win tenders, and a column named `publication_date`. 

We then store the result of the indicator calculation in a new variable called `ind_1_result`. Finally, we add the `ind_1` column to the partner's data using the `$` operator.
