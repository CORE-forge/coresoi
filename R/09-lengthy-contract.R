...
# wowo = data_out %>%
#   dplyr::filter(prepost == "post" & !is.na(contract_duration)) %>%
#   dplyr::select(cig, cf_amministrazione_appaltante, contract_duration, prepost) %>%
#   dplyr::distinct() %>%
#   # dplyr::filter(flagdivision == 1) %>%  #commented for the moment (very few data)
...
# out <- wowo %>%
#   dplyr::group_by(cf_amministrazione_appaltante) %>%
  # dplyr::summarise(ntot = dplyr::n(),
  #                  mean = mean(contract_duration, na.rm = TRUE),
  #                  median = median(contract_duration, na.rm = TRUE),
  #                  test = ifelse(ntot != 1,
  #                                wilcox.test(contract_duration, mu=grand_mean, alternative="greater")$p.value,
  #                                NA_real_) %>% suppressWarnings())

# tmp <- data_out %>%
#   filter(prepost == "post" & !is.na(contract_duration)) %>%
#   filter(cf_amministrazione_appaltante == "00040720070") %>%
#   sum(!is.na(contract_duration))
