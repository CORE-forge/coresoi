...
compute_composite <- function(indicator_list,
  ...
  # missing management
  D_norm_miss <- manage_missing(
    data = D_norm,
    missing = miss_method
  )
  # weights
  w_current <- get_weights(
    data = D_norm_miss,
...
composite_sensitivity_methods <- function(indicator_list,
  ...
  out_wide <- data.frame(
    aggregation_id = data_matrix$aggregation_id,
...
composite_sensitivity <- function(indicator_list,
  ...
  out_sens_all <- composite_sensitivity_methods(
    indicator_list,
    cutoff,
    expert_weights,
    FALSE,
  ...
  for (i in 1:n) {
    ...
    cat("########################################################################\n")
    out_sens1 <- composite_sensitivity_methods(
      indicator_list[-i],
      cutoff,
      expert_weights,
      TRUE,
