...
dimensionality_check <- function(indicator_list,
  ...
  if (dim_method == "IRT") {
    ...
    out_rasch1 <- mirt::mirt(
      data = data_matrix2[, -c(1, 2)],
      model = 1,
      itemtype = "Rasch",
    ...
    for (d in 2:max_ndim) {
      ...
      summary_rnd <- data.frame(
        replicate = paste0("rnd", 1:length(bic_rnd)),
        BIC = bic_rnd
      )
      ...
      if (bic_det < min(bic_rnd)) {
        out_mirt[[d]] <- out_mirt_det
      } else {
        out_mirt[[d]] <- best_rnd
      }
    }
